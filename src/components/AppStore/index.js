...
class AppStore extends Component {
  ...
  getApplistwithSearchInput = filterApplist => {
    const {searchInput} = this.state
    const searchResults = filterApplist.filter(eachApp =>
  ...
  render() {
    const {activeTabId} = this.state
    const filterApplist = this.getFilterAppList()

    const filterApplistWithSearchInput = this.getApplistwithSearchInput(
      filterApplist,
    )
    ...
    return (
      <div className="bg-container">
        ...
        <ul className="unorderList-element">
        ...
        <ul className="unorderList-element">
          {filterApplistWithSearchInput.map(eachItem => (
            <AppItem appList={eachItem} key={eachItem.appId} />
          ))}
        </ul>
      </div>
    )
